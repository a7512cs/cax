...
namespace OutputExcelForm
{
    ...
    public class GetExcelForm
    {
        ...
        public static bool InsertDataToMEExcel(string partNo, string cusVer, string op1)
        {
            try
            {
                foreach (KeyValuePair<DB_MEMain, IList<Com_Dimension>> kvp in OutputForm.DicDimensionData)
                {
                    if (kvp.Key.comMEMain.sysMEExcel.meExcelType == "FAI")
                    {
                        ...
                        {
                            status = Excel_FAI.CreateFAIExcel_XinWu(partNo, cusVer, op1, kvp.Key, kvp.Value);
                            if (!status)
                            {
                    ...
                    else if (kvp.Key.comMEMain.sysMEExcel.meExcelType == "FQC")
                    {
                        ...
                        {
                            status = Excel_FQC.CreateFQCExcel_XinWu(partNo, cusVer, op1, kvp.Key, kvp.Value);
                            if (!status)
                            {
                    ...
                    else if (kvp.Key.comMEMain.sysMEExcel.meExcelType == "IQC")
                    {
                        ...
                        {
                            status = Excel_IQC.CreateIQCExcel_XinWu(partNo, cusVer, op1, kvp.Key, kvp.Value);
                            if (!status)
                            {
                    ...
                    else if (kvp.Key.comMEMain.sysMEExcel.meExcelType == "IPQC")
                    {
                        ...
                        {
                            status = Excel_IPQC.CreateIPQCExcel_XinWu(partNo, cusVer, op1, kvp.Key, kvp.Value);
                            if (!status)
                            {
                    ...
                    else if (kvp.Key.comMEMain.sysMEExcel.meExcelType == "SelfCheck")
                    {
                        ...
                        {
                            status = Excel_SelfCheck.CreateSelfCheckExcel_XinWu(partNo, cusVer, op1, kvp.Key, kvp.Value);
                            if (!status)
                            {
        ...
        public static bool InsertDataToTEExcel(string partNo, string cusVer, string op1)
        {
            try
            {
                foreach (KeyValuePair<DB_TEMain, IList<Com_ShopDoc>> kvp in OutputForm.DicShopDocData)
                {
                    if (kvp.Key.comTEMain.sysTEExcel.teExcelType == "ShopDoc")
                    {
                        ...
                        {
                            status = Excel_ShopDoc.CreateShopDocExcel_XinWu(partNo, cusVer, op1, kvp.Key, kvp.Value);
                            if (!status)
                            {
