...
//         }
        ...
        public static bool SetCusVerData(Sys_Customer customerSrNo, string PartNoComboboxText, ComboBoxEx CusVerCombobox)
        {
            try
            {
                IList<Com_PEMain> comPEMain = session.QueryOver<Com_PEMain>()
        ...
        public static bool SetOpVerData(Sys_Customer customerSrNo, string PartNoComboboxText, string CusVerComboboxText, ComboBoxEx OpVerCombobox)
        {
            try
            {
                IList<Com_PEMain> comPEMain = session.QueryOver<Com_PEMain>()
                                              .Where(x => x.sysCustomer == customerSrNo)
                                              .Where(x => x.partName == PartNoComboboxText)
                                              .Where(x => x.customerVer == CusVerComboboxText)
                                              .List<Com_PEMain>();
                //CusVerCombobox.DisplayMember = "customerVer";
                //CusVerCombobox.ValueMember = "peSrNo";
                foreach (Com_PEMain i in comPEMain)
        ...
        public static bool SetOp1Data(Sys_Customer customerSrNo, string PartNoComboboxText, string CusVerComboboxText, string OpVerComboboxText, ComboBoxEx Op1Combobox)
        {
            try
            {
                Com_PEMain comPEMain = session.QueryOver<Com_PEMain>()
                                              ...
                                              .Where(x => x.partName == PartNoComboboxText)
                                              .Where(x => x.customerVer == CusVerComboboxText)
                                              .Where(x => x.opVer == OpVerComboboxText)
        ...
        public static bool SetMEExcelData(Com_PartOperation comPartOperation, string partNo, string cusVer, string opVer, string op1, ref GridPanel MEPanel)
        {
            try
            {
                ...
                foreach (Com_MEMain i in comMEMain)
                {
                    ...
                    #region 插入Panel
                    object[] o = new object[] { false, sysMEExcel.meExcelType, i.draftingVer, ""
                                                , string.Format("{0}_{1}_{2}_OP{3}資料夾"
                                                , partNo
                                                , cusVer
                                                , opVer
                                                , op1) };
        ...
        public static bool SetTEExcelData(Com_PartOperation comPartOperation, string partNo, string cusVer, string opVer, string op1, ref GridPanel TEPanel)
        {
            try
            {
        ...
        public static bool GetShopDocData(ComboBoxEx Op1Combobox, out Dictionary<DB_TEMain, IList<Com_ShopDoc>> DicShopDocData)
        {
            ...
            try
            {
                for (int i = 0; i < OutputForm.TEPanel.Rows.Count; i++)
                {
                    ...
                    sDB_TEMain.partDesc = (session.QueryOver<Com_MEMain>()
                                                    .Where(x => x.comPartOperation == (Com_PartOperation)Op1Combobox.SelectedItem)
                                                    .SingleOrDefault<Com_MEMain>()).partDescription;
