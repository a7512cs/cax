...
namespace MEUpload
{
    public partial class MEUploadDlg : DevComponents.DotNetBar.Office2007Form
    {
        ...
        private void MEUploadDlg_Load(object sender, EventArgs e)
        {
            ...
            //拆零件路徑字串取得  客戶名稱[3]、料號[4]、客戶版本[5]、製程版本[6]、製程序
        ...
        private void OK_Click(object sender, EventArgs e)
        {
            ...
            System.IO.File.WriteAllLines(string.Format(@"{0}\{1}\{2}", cMETE_Download_Upload_Path.Server_ShareStr, "OP" + sPartInfo.OpNum, "PartNameText_OIS.txt"), PartText);
                    ...
                    else
                    {
                        if (eTaskDialogResult.Yes == CaxPublic.ShowMsgYesNo("此料號已存在上一次的標註尺寸資料，是否更新?"))
                        {
                            try
                            {
                                ...
                                Com_MEMain DB_ComMEMain = new Com_MEMain();
                                DB_ComMEMain = session.QueryOver<Com_MEMain>()
